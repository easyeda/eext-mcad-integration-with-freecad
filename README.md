...
1. 点击 **保存**（`Ctrl+S`），将宏命名，保存到默认宏目录
2. 点击 **执行或双击执行**（绿色三角形按钮，或 `Ctrl+F5`）运行脚本
   ![操作流程](images/2.jpg)
3. **验证启动成功**：点击 **视图** → **面板** → 勾选 **报告视图**，在底部报告视图中应看到：
...
- 目前仅支持一对一连接。多个PCB或多个FreeCAD文
  档之间会串数据，不要同时开多个pbc并开启同步
